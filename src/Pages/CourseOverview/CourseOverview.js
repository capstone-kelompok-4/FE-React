...
export default function CourseOverview() {
  ...
  useEffect(() => {
    ...
    var configGetCourseBySpecialization = {
      method: 'get',
      url: `${BASE_URL}/courses/recommendations`,
      headers: { 
        'Authorization': `Bearer ${token}`
      }
    };
    axios(configGetAllCourses).then(res => setAllCourseData(res.data.data)).catch(err => console.log(err));
    axios(configGetCourseBySpecialization).then(res => setMyCoursesData(res.data.data)).catch(err => console.log(err));
  }, [])
    ...
    <>
      ...
      <div className="row" style={{marginLeft: "75px", marginBottom: "6rem"}}> 
        <div className="p-0 col-md-12 my-4">  
          ...
          <div className='my-5' >
            <CoursesContainer data={myCoursesData} title="My Course" showInfo={false} showProgressBar={true} className={"row row-cols-sm-1 row-cols-xl-4 row-cols-lg-3 row-cols-md-2 g-4 my-0"} searchTerm={searchTerm}/>
          </div>
          <div className='my-5' >
